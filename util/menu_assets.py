...
from util.game_actions import quit, go_to_credits, go_to_debug, go_to_infopedia, go_to_options, start_game, toggle_pause_simulation, forward, fast_forward, rewind, fast_rewind, go_to_main_menu, show_tps, hide_tps, set_tps, create_world
...
buttons: dict[MenuArea, list[Button]] = {
    ...
    MenuArea.SIMULATION: [
        ...
        Button("TPS", (615, 615), "", type=ButtonType.TOGGLE,style=ButtonStyle(height=30,width=50, scale=1.2), on_enter=show_tps, on_leave=hide_tps),
        Button("", (135, 615), "",style=ButtonStyle(height=30,width=30, scale=1.2), on_enter=lambda button: create_world(seed=States.world.get_seed() if States.world else None))
    ]
}
States.fast_rewind = buttons[MenuArea.SIMULATION][0]
States.rewind = buttons[MenuArea.SIMULATION][1]
States.pause = buttons[MenuArea.SIMULATION][2]
States.forward = buttons[MenuArea.SIMULATION][3]
States.fast_forward = buttons[MenuArea.SIMULATION][4]
States.tps_button = buttons[MenuArea.SIMULATION][6]
